...
r = 10;
select = '300k';
tck_file = sprintf('%s/sphere_%d_select_%s_NAc.tck', targeting_path, r, select);
...
cmd = sprintf(['tckgen -act %s/5tt_coreg_hsvs.mif -backtrack -include %s/include_spec.nii ' ...
    ...
    mrtrix_path, targeting_path, x(1), y(1), z(1), r, x(2), y(2), z(2), r, mrtrix_path, tck_file, select, cores);
...
for k = 1 : size(target_dwi, 1)
    ...
    cmd = sprintf(['tckgen -act %s/5tt_coreg_hsvs.mif -backtrack -include %s/include_spec.nii ' ...
        ...
        '-step 0.5 -crop_at_gmwmi -nthreads %d -quiet -force'], mrtrix_path, targeting_path, x, y, z, r, mrtrix_path, tck_file, seeds, cores);
    system(cmd);
end
